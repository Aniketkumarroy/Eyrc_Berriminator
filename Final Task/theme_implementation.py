...
# Functions:	    [ call_open_close, berry_positions, target_berry_position, wait, communicate, rotategripper, pluck_and_drop, M, delay, slide, navigate_and_pluck, arm_initial, nearest_point, drop, collect, read_force_sensor, path_planner, centering, send_identified_berry_data ]	
# Global variables:	
...
def pluck_and_drop(berry,berry_data,threshold, client_id,gripper):
	...
	code, joint2 = sim.simxGetObjectHandle(client_id,"robotic_arm_pj_23",sim.simx_opmode_blocking)
	n_berry = berry_data[berry][0] # required berries
	box_number = berry_data[berry][1]
	pt = basket[box_number]
	safe_height = -0.17317 # safe height to hover safely
	if n_berry > 0: #if berry is required
			while True:
				...
				m = M(target[0], target[2], 0)
				x_norm, z_norm = target[0]/m, target[2]/m #direction cosine of berry with respect to vision sensor
				communicate(client_id,'robotic_arm','Parameter',[],[threshold*x_norm,safe_height,threshold*z_norm],'vs')
				wait(client_id,"M")
				communicate(client_id,'robotic_arm','Parameter',[],[threshold*x_norm,target[1],threshold*z_norm],'vs')
				wait(client_id,"M")
				communicate(client_id,'robotic_arm','Parameter',[],target,'vs')
				wait(client_id,"M")
				call_open_close(client_id, "close")
				wait(client_id,"G")
				tip_to_berry = M(threshold*x_norm - target[0],threshold*z_norm - target[2],0) # distance of tip from berry to be plucked
				if tip_to_berry < 0.15:
					# berry is too close, moving the bot backward to pluck strong berry
					x, y = slide(client_id,d)
					flag = 1
				else:
					# just pulling the arm
					sim.simxSetJointTargetPosition(client_id,joint2,0.0,sim.simx_opmode_oneshot)
					delay(1.0)
				communicate(client_id,'robotic_arm','Parameter',[],[threshold*x_norm,safe_height,threshold*z_norm],'vs')
				if flag != 0:
					# restoring to bot's initial position if bot has moved backwards
...
#Function Name:M
#Input:x, y, z(x, y and z coordinates of a point)
#Output:(distance of that point from their origin)
#Logic:uses euclidian formula for distance calculation
#Example call: distance = M(x, y, z)

M = lambda x, y, z: math.sqrt(x**2 + y**2 + z**2)

###########################################################

#Function Name:delay
#Input:T(time to wait)
#Output:NONE
#Logic:uses while loop and time function of time module
#Example call: delay(T)
...
#Function Name:slide
#Input:client_id, d(the required offset for the bot to step behind)
#Output:x, y(the initial position of bot before stepping behind)
#Logic:uses navigation command to move the bot backward
#Example call: x, y = slide(client_id, d)

def slide(client_id,d):
	...
	# getting the required back co-ordinates based on bot's orientation
	if (alpha == 0) or (math.fabs(math.fabs(alpha)-math.pi) <= 0.01):
		if alpha == 0:
			break_offset = [x,y-d]
		else:
			break_offset = [x,y+d]
		...
		if alpha > 0:
			...
			break_offset = [x-d,y]
	# moving the bot slight backwards to pluck strong berries
...
#Function Name:arm_initial(client_id)
#Input:client_id
#Output:NONE
#Logic:uses joint object handle and set its base position
#Example call: arm_initial(client_id)

def arm_initial(client_id):
	code, joint = sim.simxGetObjectHandle(client_id,"robotic_arm_rj_r1",sim.simx_opmode_blocking)
	# bringing arm to normal position
	sim.simxSetJointTargetPosition(client_id,joint,0.0,sim.simx_opmode_oneshot)
...
def theme_implementation_primary( client_id, rooms_entry):
	...
	#reversing the given entry list to reduce the path length
	rooms_entry.reverse()
...
if __name__ == "__main__":
	...
	rooms_entry = [(2,3),(5,8),(3,2)]     # example list of tuples
