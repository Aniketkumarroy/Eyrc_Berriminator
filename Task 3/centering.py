...
def detect_qr_codes(transformed_image,resolution,flag):
    ...
    for qrcode in codes:
        ...
        if flag:
            ...
            approx = np.array(qrcode.polygon,np.uint8)
            add = np.sum(approx,axis=1)
            upper = approx[np.argmin(add)]
            lower = approx[np.argmax(add)]
        slope = (upper[1]-lower[1])/(upper[0]-lower[0])
    return qr_codes,(cX,cY),upper,lower,slope
##############################################################
...
def task_3_primary(client_id, target_points):
    ...
    centering(client_id,w,True,[1,1])
    # ax = 1
    # ay = 1
    # while True:
    #     img_lis, reso, code = get_vision_sensor_image(client_id)
    #     img = transform_vision_sensor_image(img_lis,reso)
    #     qr_code,(cx,cy) = detect_qr_codes(img)
    #     if len(qr_code) > 0:
    ...
    #             set_bot_movement(client_id,w,0,ax*error_x,0)
    #             continue
    #         set_bot_movement(client_id,w,ay*error_y,0,0)
    #         if abs(error_y) < 0.005:
    #             break
##############################################################


def centering(client_id,wheel_joints,flag,point):
	ax = 1
	ay = 1
	aa = 2
	Bool = True
	while True:
		img_lis, reso, code = get_vision_sensor_image(client_id)
		img = transform_vision_sensor_image(img_lis,reso)
		qr_code,(cx,cy),up,low,slope = detect_qr_codes(img,reso,Bool)
		if len(qr_code) > 0:
			error_x, error_y = (cx-(reso[1]//2),(reso[0]//2)-cy)
			error_x, error_y = error_x*scale, error_y*scale
			cv2.circle(img,(cx,cy),4,(255,0,0),-1)
			cv2.circle(img,up,4,(0,0,255),-1)
			cv2.circle(img,low,4,(0,0,255),-1)
			cv2.circle(img,(reso[1]//2,reso[0]//2),4,(0,255,0),-1)
			cv2.imshow("output",img)
			cv2.waitKey(1)
			...
			if abs(error_y) > 0.005:
				...
				continue
			# if 0.999 > abs(slope) or abs(slope) > 1.001:
			# 	# alpha = math.atan(slope)
			# 	set_bot_movement(client_id,wheel_joints,0,0,aa*(1-slope))
			# 	print(slope)
			# 	continue
			# if Bool == False:
